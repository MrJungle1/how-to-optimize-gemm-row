                                       ...
                                       double *c, int ldc )
{
  ...
  for ( i=0; i<m; i+=4 ){        /* Loop over the columns of C, unrolled by 4 */
    for ( j=0; j<n; j+=4 ){        /* Loop over the rows of C */
      /* Update C( i,j ), C( i,j+1 ), C( i,j+2 ), and C( i,j+3 ) in
	 one routine (four inner products) */
      PackMatrixA( k, &A( i, 0 ), lda, &packedA[ i*k ] );
      if ( j == 0 ) AddDot4x4( k, &A( i,0 ), lda, &B( 0,j ), ldb, &C( i,j ), ldc );
    }
  }
}
...
void AddDot4x4( int k, double *a, int lda,  double *b, int ldb, double *c, int ldc )
{
  ...
  for ( p=0; p<k; p++ ){
    ...
    a_2p_vreg.v = _mm_loaddup_pd( (double *) a_2p_pntr++ );   /* load and duplicate */
